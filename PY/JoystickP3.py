...
def keyDown(key):               #what to do if key pressed. takes value from handleJoyStickAsArrowKeys
    keysDown[key] = True        #adds key to KeysDown listi
    pydirectinput.keyDown(key)  #runs pydirectinput using kiey from (argument)
    #print('Down: ', key)       #remove '#' from print to teist data streami
...
def handleJoyStickAsArrowKeys(x, y, z):      #note that the x and y directions are swapped due to the way I orient my thumbstick
    ...
    if z == 1:          #z argument is JSButton in this case. 1 is button pressed
        keyDown('d')    #key to be pressed with Joystick button. Change to any key
    else:
        keyUp('d')      #0 is button not pressed
